...
from tpcwithdnn.eval import root_mean_squared_error
...
from tpcwithdnn.utilities_dnn import u_net, simple_net
...
class DnnOptimiser:
    ...
    def train(self):
        ...
        model = simple_net((self.grid_phi, self.grid_r, self.grid_z, self.dim_input),
        ...
        model.compile(loss=root_mean_squared_error, optimizer=Adam(lr=self.adamlr),
                      metrics=[self.metrics, MeanAbsoluteError(), 
                      MeanAbsolutePercentageError(), RootMeanSquaredError()]) # Mean squared error and so on...
